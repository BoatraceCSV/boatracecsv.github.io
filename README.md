...
**追跡方法**: 同じ `レースコード` で3つのファイルを紐付けることで、選手情報から展示会走行、本レース結果までの完全な追跡が可能です。

**例**: レースコード「201601150001」で検索すると：
1. **Programs** から → 参加選手のプロフィール・成績
2. **Previews** から → 展示会での走行タイム・当日コンディション
3. **Results** から → 最終順位・払戻金情報

これらを組み合わせることで、レースの準備段階から結果までの全体像を把握できます。
