...
各レース場が独自に計測している展示走行データです。`race.boatcast.jp` が配信している TSV をソースにしています。計測項目はレース場によって異なり、宮島や大村など多くの場では「一周／まわり足／直線」の3項目、住之江・尼崎・徳山では「一周／まわり足」の2項目、桐生は「半周ラップ／まわり足／直線」を計測しています。列の意味が場ごとに違うため、`計測項目1〜3` 列にその行の場が計測した指標名を入れる形式で1ファイルに収めています。
...
- `レース場` (17): レース場コード（2桁 zero-padded、例 `'01'`〜`'24'`）。Race Cards / Recent Form の `レース場コード` 列と同じ形式で JOIN 可能
- `レース回` (01R): 当日の何レース目か

**計測メタ情報**:
- `ステータス`: `1` = 正常に計測済み、`2` = このレースは計測できなかった、`0` = 計測中（前のレースが未終了等の稀なケース）、空欄 = データ未取得。`2` / `0` の行では艇ごとの値が空になります
- `計測数`: そのレース場の計測項目数（`2` または `3`）
- `計測項目1` / `計測項目2` / `計測項目3`: その場の計測指標名
  - **3項目場（18場）**: 一周 / まわり足 / 直線（宮島・大村・芦屋など）
  - **2項目場（3場）**: 一周 / まわり足 — 場12=住之江、場13=尼崎、場18=徳山。`計測項目3` と各艇の `値3` は空
  - **半周ラップ場（1場）**: 半周ラップ / まわり足 / 直線 — 場01=桐生のみ
