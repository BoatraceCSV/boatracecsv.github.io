...
### Programs (出場艇情報)
**ファイルパス**: `data/programs/daily/YYYY/MM/DD.csv`
**URL**: https://boatracecsv.github.io/data/programs/daily/2026/01/01.csv
...
### Previews — 歴史アーカイブ (展示会情報・統合フォーマット)
**ファイルパス**: `data/previews/daily/YYYY/MM/DD.csv` (2016-01-15 〜 2026-05-12)
**URL**: https://boatracecsv.github.io/data/previews/daily/2026/01/01.csv

> ⚠️ **更新停止済み (2026-05-12 まで)**。本ファイル群は履歴アーカイブとして残置されており、新規日付の追加はありません。当日以降のデータはデータソース単位の per-source CSV (`tkz` / `stt` / `sui` / `original_exhibition`、次節 *Realtime Preview* 参照) に分割書き出しされます。歴史日付の特徴量計算は `scripts/backfill_realtime_from_daily.py` により直近6ヶ月分が per-source 形式に再構成済みです。

展示会での各艇の走行データです。レース当日の朝に実施される展示会での情報を含みます。

**データソース**: 2026年4月〜5月初頭、`race.boatcast.jp` が配信する TSV (`bc_j_tkz` / `bc_j_stt` / `bc_rs1_2`) を組み合わせて生成していました（それ以前は `www.boatrace.jp` の HTML をスクレイプ）。これに伴い気象6項目 (`風速(m)` / `風向` / `波の高さ(cm)` / `天候` / `気温(℃)` / `水温(℃)`) は、従来の「展示会終了時点」の値ではなく **「当該レースの確定時点 (`bc_rs1_2`) の値」** を記録します。当日内で風速・風向・気温が時間経過で変化するため、過去データと数値が ±数単位ずれる場合があります（艇別の体重・展示タイム・チルト・スタート展示の値は影響を受けません）。
...
- 気象6列: `天候` / `風向` / `風速(m)` / `波の高さ(cm)` / `気温(℃)` / `水温(℃)`
  - コード値の体系は Previews / Realtime Preview の `sui` と同一
...
6. **Previews** から → 展示会での走行タイム・当日コンディション
7. **Realtime Preview** から → 締切5分前の直前スナップショット（時系列で複数ソース）
8. **Strength Index** から → 6 枠分の強さポイント(偏差値)と要素別寄与の内訳
9. **Realtime Results** から → 締切後5〜30分の準リアルタイム結果(着順・決まり手・ST・気象)
10. **Results** から → 最終順位・払戻金・実際の進入コース・ST
...
python scripts/program.py --force                                      # B-file 由来 (当日 Programs)
...
├── programs/
│   ├── daily/YYYY/MM/DD.csv                # B-file 由来の出走表 (Programs)
...
├── previews/
│   ├── daily/YYYY/MM/DD.csv                # 歴史アーカイブ (2016-2026/05/12、更新停止)
...
# Programs (B-file, 当日出走表)
python scripts/program.py --force
...
> Preview (展示会) は per-race 締切直前にしか取れないため、過去日の単発再 fetch は不可。詳細は次節 *Realtime Preview Scraper* を参照。
...
- **`preview-realtime.yml`** — `workflow_dispatch` manual fallback only. The production schedule (JST 08:00-22:59, every 5 minutes) has been migrated to **Cloud Scheduler + Cloud Run Jobs** because GitHub Actions cron was being throttled. Two passes per invocation:
  ...
  - **Cloud Run Jobs 構成**: `boatrace-487212/asia-northeast1` の Cloud Scheduler `preview-realtime-daytime` (`*/5 8-22 * * *`, Asia/Tokyo) が Cloud Run Job `preview-realtime` を発火します。詳細は [`infra/README.md`](infra/README.md) を参照。
