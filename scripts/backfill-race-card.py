#!/usr/bin/env python3
"""Backfill race-card detail data (出走表詳細) for a date range.

Usage:
    # Default: from 2025-05-03 (earliest available on race.boatcast.jp)
...
NOTES:
    * race.boatcast.jp only carries bc_j_str3 from 2025-05-03 onwards.
      Earlier dates return HTTP 403 (no file). The default --start-date
      reflects this.
    * By default, days whose CSV already exists are skipped. Use --force to
      re-scrape and overwrite them.
    * The script issues up to 12 requests per open stadium per day. On a
      24-stadium peak day that's ~288 requests; combined with the default
      rate-limit (1s/3s) a full backfill of ~12 months takes several hours.
...
# Earliest date confirmed empirically: 2025-05-03 is the first hit; 2025-05-02
...
def _collect_actual_races(
    ...
    """Download the B-file for ``date_str`` and return ``{(stadium, race)}``."""
    actual_races = set()
    ...
    logging_module.info(
        "backfill_race_card_b_file_downloading",
        date=date_str,
        url=b_file_url,
    )
    ...
    if not b_content:
        logging_module.warning(
            "backfill_race_card_b_file_missing", date=date_str
        )
        return actual_races
    ...
    try:
        ...
        logging_module.warning(
            "backfill_race_card_b_file_extract_error",
            date=date_str,
            error=str(e),
        )
        return actual_races
    ...
    try:
        ...
        logging_module.warning(
            "backfill_race_card_b_file_parse_error",
            date=date_str,
            error=str(e),
        )
        return actual_races
...
def _process_one_date(
    ...
    for stadium_code, race_number in sorted(actual_races):
        try:
            ...
            logging_module.warning(
                "backfill_race_card_scrape_error",
                date=date_str,
...
def _parse_arguments():
    parser = argparse.ArgumentParser(
        description="Backfill race-card detail data across a date range"
    )
    parser.add_argument(
        "--start-date",
        type=str,
...
def main():
    ...
    if datetime.strptime(args.start_date, "%Y-%m-%d") < datetime.strptime(
        ...
        print(
            f"Warning: race.boatcast.jp has no bc_j_str3 data before "
        ...
        if day_stats["skipped_existing"]:
            ...
            if day_stats["csv_written"]:
                ...
                if args.push and not args.dry_run:
                    ...
                    message = f"Backfill boatrace race card data: {date_str}"
    ...
    logging_module.info("backfill_race_card_complete", totals=total)
