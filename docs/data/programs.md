...
- [Waku10](#waku10-枠番別過去10走) — 枠番別過去10走
- [Monthly Schedule](#monthly-schedule-月間開催日程) — 月間開催日程(全24場)
...
- [Motor History](#motor-history-モーター履歴) — モーター履歴(節ごとの使用者・着順列)
...
- **ファイルパス**: `data/programs/waku10/YYYY/MM/DD.csv`
- **URL**: https://boatracecsv.github.io/data/programs/waku10/2026/07/19.csv

`race.boatcast.jp` の `bc_j_waku10` を起源とするデータです。各選手が **今回と同じ枠番** で出走した直近10走の着順・進入コース・グレードと、枠番別の集計値(勝率・平均ST・平均スタート順)を含みます。1行 = 1レース(4 メタ列 + 6艇 × 34列 = 208列)。

### 列の詳細説明

**基本情報**: `レースコード` / `レース日` / `レース場コード` / `レース回`(Race Cards と同形式)

**艇N 枠番別集計** (N=1..6):

- `艇N_選手名`: 選手名(全角スペースは半角1個に正規化)
- `艇N_枠番別勝率`: この枠番での勝率
- `艇N_枠番別平均ST`: この枠番での平均スタートタイミング
- `艇N_枠番別平均スタート順`: この枠番での平均スタート順(1.0〜6.0)

**艇N 過去10走** (各走3列、`過去1走` = 前走 → `過去10走` = 10走前の新しい順):

- `艇N_過去{n}走_着順`: 着順または特殊トークン。半角1〜6 / `F` / `L` / `欠` / `落` / `沈` / `転` / `不` / `エ` / `失` / `妨`(全角は半角に正規化済み)
- `艇N_過去{n}走_進入`: 実際の進入コース (1-6)。**空欄は枠なり進入**(ソースが枠と異なる場合のみ値を持つ)
- `艇N_過去{n}走_グレード`: そのレースのグレード。`IP`(一般)/ `G3` / `G2` / `G1` / `SG`

出走歴が10走に満たない場合、古い側のスロットは全列空欄です。

**データソース**: `https://race.boatcast.jp/hp_txt/{jo}/bc_j_waku10_{ymd}_{jo}_{race}.txt`(`bc_j_str3` と同時に毎朝取得)

> **用途**: 枠番適性の特徴量(同一枠での実績・ST 安定性)。Race Cards と `レースコード` + 艇番で JOIN して利用します。

---

## Monthly Schedule (月間開催日程)

**月間開催日程**

- **ファイルパス**: `data/programs/monthly_schedule/YYYY/MM.csv`
- **URL**: https://boatracecsv.github.io/data/programs/monthly_schedule/2026/07.csv

`race.boatcast.jp` の `bc_mon_2_{YYYYMM}_{jo}` を起源とする全24場の節(開催)一覧です。ソースファイルは取得月を超えて **数ヶ月先の節まで** 含むため、将来の開催予定の先読みに使えます。毎日再取得して **月ファイルを上書き** するため、月中の日程変更は git 履歴で追跡できます。

### 列の詳細説明

- `場コード`: `01`〜`24`(2桁ゼロ詰め)
- `節開始日` / `節終了日`: `YYYY-MM-DD`
- `グレード`: `IP`(一般)/ `G3` / `G2` / `G1` / `SG`
- `タイトル`: 開催タイトル(全角)
- `レース数`: 1日のレース数(例 `12R`)

行順は 場コード → 節開始日 の昇順です。

> **用途**: 開催スケジュールの先読み、節単位の集計キー(節開始日は Recent Form・モーター履歴系のキーとしても機能)
...
## Motor History (モーター履歴)

**モーター履歴**

- **ファイルパス**: `data/programs/motor_history/YYYY/MM/DD.csv`(日付 = **基準節終了日**。実行日ではない点に注意)
- **URL**: https://boatracecsv.github.io/data/programs/motor_history/2026/07/12.csv

`race.boatcast.jp` の `bc_mrireki_{節終了日}_{jo}` を起源とする、モーター番号ごとの **節単位の使用履歴** です。各ファイルはその節終了日を基準としたスナップショットで、モーターごとに直近約3節分の {使用期間, グレード, タイトル, 使用者名, 着順列} を含みます。1行 = 1(モーター × 過去節)。

Motor Stats (`bc_mdc`) が期集計値のみなのに対し、こちらは **どの選手がいつ使ってどんな着順だったか** の生履歴が取れます。また `bc_mdc` と異なりソースが過去節分も保持(実測 1ヶ月以上)しているため、導入時点からの短期バックフィルが可能です。

### 列の詳細説明

- `場コード`: `01`〜`24`(2桁ゼロ詰め)
- `基準節終了日`: ソースファイルのキー(`YYYY-MM-DD`)。同じ日に節が終了した複数の場が同一ファイルに入ります
- `モーター番号`: 整数
- `使用開始日` / `使用終了日`: その節の期間(`YYYY-MM-DD`)
- `グレード`: `一般` / `ＧⅢ` / `ＧⅡ` / `ＧⅠ` / `ＳＧ`(全角。Recent Form と同じ表記)
- `タイトル`: 開催タイトル
- `使用者名`: その節でモーターを使った選手(全角スペースは半角1個に正規化)
- `着順列`: 節内全走の生着順文字列。トークンは Recent Form の `着順列` と同一(全角 `１`〜`６` / `Ｆ` / `Ｌ` / `欠` / `転` / `妨` / `落` / `[N]`=優勝戦N着 / 全角スペース=日区切り)。末尾パディングは除去済み

### 取得・更新ルール

- `motor-stats.py`(daily-sync)が開催中の各場について、`bc_mon_2` から「実行日より前に終了した直近の節」の終了日を導出し、該当キーの `bc_mrireki` を取得します
- ファイルはキーごとに静的なため、**同じ場が既に記録済みならスキップ**(冪等)。新しい節が終わると次の実行で新キーのファイルに追記されます

**データソース**: `https://race.boatcast.jp/hp_txt/{jo}/bc_mrireki_{節終了日yyyymmdd}_{jo}.txt`

> **用途**: モーターの素性追跡(直近の使用者の腕を補正したモーター評価、乗り換わり分析)。`場コード` + `モーター番号` で Motor Stats / Race Cards の `艇N_モーター番号` と JOIN して使用。
