...
> **展開優位pt (`tenkai`) について**: 2026-05-30〜06-12 の間、v2_tenkai は本成分を採用して
> いたが A君予想を回収率で下回ったため撤去した。計算ロジック
> ([`tenkai_yui_pt()`](../../scripts/boatrace/index_features.py))と `COMPONENT_LABELS_REGISTRY`
> のラベルは将来の再利用に備えて残してあるが、**現在どの active 予想者の `component_keys`
> にも含まれていない**ため index CSV には出力されない。
