...
> 回収率は「control 比で -7pt 級の劣化を検知する**ガードレール**」として使う。
...
「このレースは荒れるか」を確率で出す派生データ。**予想者に紐づかない独立の指標**で、
...
`scripts/build_kimarite.py` が `results/realtime` × `previews` × `race_cards` の
**全履歴**から学習する。
...
**推論側は sklearn を使わない**(この CSV から softmax を直接計算する)。
`daily` 版は `race_cards` + 枠なり仮定のみ、`realtime` 版は展示進入・展示・気象込み。

### `data/estimate/kimarite/YYYY/MM/DD.csv`(日次 + 直前)

`scripts/build_kimarite_probs.py` が出力する。レース × 状態 で 1 行。

| 列 | 説明 |
| --- | --- |
| `レースコード` / `レース日` / `レース場コード` / `レース回` | 識別子 |
| `状態` | `daily` / `realtime` |
| `荒れ度` | **`1 − P(逃げ_1)`**。これが主表示 |
| `P_{クラス}` × 32 | 各セルの確率(小数 6 桁) |

ホールドアウト実測(41,740 レースの末尾 20%):

| モデル | log-loss | ベースレート | 改善 | P(逃げ) 予測平均 / 実測 |
| --- | ---: | ---: | ---: | --- |
| realtime | 1.8331 | 2.0107 | **+0.1776 nat** | 0.518 / 0.521 |
| daily | 1.8734 | 2.0107 | +0.1373 nat | 0.528 / 0.521 |

> **`荒れ度` は使えるが `argmax` は使えない。** 確率としての校正は良い
> (予測平均と実測のズレ 0.3pt)が、「このレースは○○で決まる」という
> **レース単位の決まり手予測は条件付きでもベースレートを超えない**
> (設計書 §14.2)。94.5% のレースで最有力が「逃げ」になるため、
> **必ず全クラスの確率を並べて出し、1 つを強調しないこと**。
...
| 列 | 説明 |
| --- | --- |
| `セル` | `まくり差し_3` 等(`boatrace.kimarite.CELLS`) |
| `2着コース` / `3着コース` | 1〜6 |
| `n` | 観測レース数 |
| `確率` | セルごとに合計 1 |

収縮は周辺分布の積へ **k=150**。ホールドアウト(3,792 レース)の条件付き log-loss で
選定した(k=0 → 2.8148 / **k=150 → 2.8024** / k=1500 → 2.8051)。
スジ表と違い**収縮が効く**のは、セルが 32 個に分かれてサンプルが減るため。

> **スジ表との違い**: スジ表は 1着コースだけで条件付けるが、こちらは
...
荒れ度メーターの校正監視。`scripts/build_kimarite_calibration.py` が
予測 (状態=realtime) と実績を突き合わせて出す。

| 列 | 説明 |
| --- | --- |
| `予測帯` | `0.4-0.5` 等。`合計` 行が 1 本入る |
| `n` | そのレース数 |
| `予測荒れ度` / `実測荒れ度` | 予測の平均と、実際に 1コース以外が 1着だった率 |
| `差pt` | 実測 − 予測。**KPI は各帯で ±5pt 以内**(設計書 §7) |
| `logloss` | 32 クラスの log-loss。**Phase 2 (A案 vs B案) の主判定指標** |

> **log-loss も出す理由**: 回収率で穴予想どうしを比べると差の検出に 8 ヶ月
> かかるが、log-loss なら数週間で決着する(設計書 §13.3)。判定に使う値を
> 運用データから取れるようにしてある。

> **クラスの凍結**: 2026-08-12 時点の全履歴で `n >= 60` だった 26 セル +
> 受け皿の `その他_{1着コース}` 6 個 = 32。月次再学習で増減させない
> (`cell_coef.csv` のスキーマが変わるため)。1着コースは**展示進入**基準なので、
> 展示と本番で進入が変わったレースでは `逃げ_2` のような組合せが出る
> (実データで n=188)。定義どおりの挙動なので捨てない。
