...
| `monthly-weights` | JST 06:00 / 毎月 1 日 | `.github/workflows/monthly-weights.yml` (移行完了後に削除) | 直近 6 ヶ月の特徴量から場ごとの 5 要素重みを再計算し `data/estimate/stadium/index_weights/YYYY-MM.csv` を更新 (daily-sync 直前に走らせ、境界日の daily/realtime index を同一 weights で計算) |
...
| ファイル | 役割 |
| --- | --- |
| `infra/Dockerfile` | Python 3.11-slim ベースの実行イメージ (preview-realtime / daily-sync / monthly-weights 共用) |
...
| `infra/run-monthly-weights.sh` | monthly-weights Job のエントリポイント (clone → sparse-checkout 8 ヶ月分 → build_weights.py → commit) |
| `infra/cloudbuild.yaml` | Cloud Build パイプライン (build → push → 3 job 更新) |
| `infra/.dockerignore` | ビルドコンテキスト最小化 |
| `infra/daily-sync-migration-plan.md` | daily-sync を GHA から Cloud Run に移行する計画書 (移行完了後に削除可) |
| `infra/monthly-weights-migration-plan.md` | monthly-weights を GHA から Cloud Run に移行する計画書 (移行完了後に削除可) |
...
| `.boatrace/` | 実行時設定 (load_config) |
| `data/estimate/stadium/` | win_rate.csv / sui_params.csv (build_weights 入力) + index_weights/ (出力先) |
| `data/results/daily/<YM>/` × 8 | 着順 (target = 7 - 着順) |
| `data/programs/race_cards/<YM>/` × 8 | レース宇宙 (universe) |
| `data/programs/recent_national/<YM>/` × 8 | recent 特徴量 |
| `data/programs/recent_local/<YM>/` × 8 | recent 特徴量 |
...
詳細な移行計画 (背景、トレードオフ、Open Questions の決定理由) は
`infra/monthly-weights-migration-plan.md` を参照。
...
> 1 日抜けるトレードオフは許容済み。詳細は migration-plan §5 参照。
...
# git に commit が積まれたか確認
gh api /repos/${GITHUB_REPO}/commits?path=data/estimate/stadium/index_weights --jq '.[0:3] | .[] | {sha, message: .commit.message}'
...
差分が無かった場合は `Pushed weights ...` の代わりに
`No weights changes to commit for ...` が出る。

### 4. 旧 GHA の停止と Scheduler 再開 (順序重要)

両方が同月の `data/estimate/stadium/index_weights/YYYY-MM.csv` を生成すると
...
monthly-weights の出力は単一 CSV (`data/estimate/stadium/index_weights/YYYY-MM.csv`)
...
* **3 Job の時間帯重なり**: 毎月 1 日のタイムライン:
  - JST 06:00: `monthly-weights` 起動 (実測 N 分、初回確認後に再評価)
  - JST 07:30: `daily-sync` 起動 (実測 ~22 分)
  - JST 08:00–22:55: `preview-realtime` 5 分毎
  monthly-weights は data/estimate/stadium/index_weights/ のみを書き、
  daily-sync は data/estimate/index/ や data/results/ などを書く。
  対象ファイルが重ならないため git push 競合は通常起きない。万一
  monthly-weights が 07:30 を跨いだ場合でも、daily-sync の commit 時に
  pull-rebase で吸収される。
