...
> **訂正**: 当初「`preview-realtime-runner` SA に `roles/run.invoker` を追加」と書いたのは誤り。Runner SA は Job が実行されるときの ID であり、Scheduler から呼ぶ ID ではない。**Cloud Scheduler は別途 Invoker SA (`preview-realtime-invoker`) を使って `jobs:run` API を叩く** (既存 README §3, §7 参照)。

- **Runner SA**: `preview-realtime-runner` をそのまま再利用する。
  - Secret Manager (`github-token`) read 権限、`roles/logging.logWriter`、`roles/storage.objectAdmin` (GCS バケット `boatrace-realtime-data-boatrace-487212`)、`roles/pubsub.publisher` (`fun-site-realtime-completed` トピック) は **preview-realtime 用に付与済み** (Runner SA が同じなので追加付与は不要)。
- **Invoker SA**: `preview-realtime-invoker` を再利用する。新規 SA は作らない (1 SA で複数 Job を Scheduler 起動するシンプル構成)。
  - 追加で必要な唯一の binding:
    ```bash
    ...
    ```
- **Cloud Build SA**: `roles/run.developer` + Runner SA への `roles/iam.serviceAccountUser` は preview-realtime 用に付与済み。daily-sync Job も同じ Cloud Build パイプラインで deploy するため**追加付与は不要**。
- **Open Question 3 対応**：`workflow_dispatch` UI は廃止するため、GHA → GCP の認証経路は不要。手動実行は `gcloud run jobs execute daily-sync --region=asia-northeast1 --update-env-vars=RUN_DATE=2026-05-01` を開発者端末から実行する運用に統一する。
...
2. **PR #2（IAM・Scheduler ドキュメント追記）**：`infra/README.md` に daily-sync 用の追加手順を追記。実行は手動 gcloud:
   - `preview-realtime-invoker` SA に `roles/run.invoker`（daily-sync Job スコープ）追加
   - Cloud Scheduler ジョブ `daily-sync` を JST 07:30 / `--paused` で作成
   - 既存の Runner SA / Cloud Build IAM はそのまま流用（追加なし）
